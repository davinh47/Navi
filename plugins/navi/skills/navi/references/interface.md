...
before reserving a fresh packet. In standard mode, minimum dispatch spacing is 30 seconds, with one worker in flight and at
most six dispatch attempts per rolling hour; no duplicate evidence or automatic failure retries. Cooldown candidates wait
...
Every automatic and manual review, including followups and failures, shares the same per-conversation review
hourly review limit: six in standard mode, four in long-task mode, plus any explicit total ceiling.
...
`supervisor.py status` shows budget, dispatch health, reminder history and followup judgments. Lost workers
are visible after 180 seconds and require explicit disable/re-enable to recover; this never resets budgets.
...
```sh
python3 scripts/report.py --data-directory ABS --session ACTUAL_SESSION_ID --format markdown
```

`--format json` includes full retained sources and cited evidence. `--output ABS_NEW_FILE` explicitly exports
...
This allows at most 4 review reservations and 4 dispatch attempts per rolling hour, at least 300 seconds
between automatic dispatches, and at least 300 seconds between reminder attempts. Standard mode uses
6/hour, 30-second dispatch spacing and 60-second reminder cooldown. No lifetime dispatch/reminder quota,
wall-clock cutoff, goal cancellation or counter reset. Released rate capacity resumes on the next Hook.
...
remaining count (null means no cumulative ceiling), hourly gate and explicit budget exhaustion. Never
forget/recreate task state to renew a ceiling. Manual calls also share the hourly review reservation limit.
