...
FINAL_WAIT_SECONDS = 15
WAIT_POLL_SECONDS = 0.25


def generate(data, session, recorded_only=False, final=False):
    started=time.monotonic()
    target_ids=None;worker_token=None;task_id=None
    while True:
        with locked(data,session) as (path,state):
            wait=None
            if final:
                jobs=(state or {}).get('reviews',{})
                worker=(state or {}).get('supervisor',{}).get('worker') or {}
                if target_ids is None:
                    task_id=(state or {}).get('task_id')
                    target_ids={k for k,j in jobs.items() if j['status'] in ('queued','running')}
                    worker_token=worker.get('token')
                # Follow an already-dispatched worker across its reservation boundary,
                # but never extend the wait to an unrelated, newly scheduled review.
                target_ids.update(k for k,j in jobs.items() if worker_token
                                  and j.get('supervision_token')==worker_token)
                pending=sorted(k for k in target_ids if jobs.get(k,{}).get('status') in ('queued','running'))
                missing=sorted(target_ids-jobs.keys())
                worker_pending=bool(worker_token and worker.get('token')==worker_token
                    and not any(j.get('supervision_token')==worker_token for j in jobs.values()))
                elapsed=time.monotonic()-started
                status=('task_changed' if (state or {}).get('task_id')!=task_id else
                        'records_unavailable' if missing else
                        'not_needed' if not target_ids and not worker_token else
                        'finished' if not pending and not worker_pending else
                        'timed_out' if elapsed>=FINAL_WAIT_SECONDS else 'waiting')
                wait={'status':status,'max_seconds':FINAL_WAIT_SECONDS,'elapsed_seconds':round(elapsed,3),
                    'review_ids':sorted(target_ids),'pending_review_ids':pending,
                    'missing_review_ids':missing,'dispatch_pending':worker_pending,
                    'outcomes':{k:jobs.get(k,{}).get('status','unavailable') for k in sorted(target_ids)},
                    'scope':'existing_reviews_only','new_review_calls':0}
            if not wait or wait['status']!='waiting':
                ...
                # Refresh only the final snapshot, never persist observations or reserve calls.
                result=build(state,path.with_suffix('.gap').exists(),refreshed,allowed)
                if state:
                    from supervisor import runtime_status
                    result['runtime']=runtime_status(data,state,path)
                if wait:result['final_wait']=wait
                result['user_summary']=brief(result)
                return result
        # Never sleep with the task lock held: the reviewer needs it to publish.
        time.sleep(min(WAIT_POLL_SECONDS,max(0,FINAL_WAIT_SECONDS-(time.monotonic()-started))))


def final_wait_lines(report):
    wait=report.get('final_wait')
    if not wait:return []
    text={
        'not_needed':'没有待等待的已有评估，直接生成报告；未新增评估调用。',
        'finished':'报告开始时的已有评估已结束或调度已退出；失败、取消及结果是否过期见下方，不代表最终审查通过。',
        'timed_out':'已达等待上限；最后一次已有评估尚未返回可用结论，本报告未包含其未返回的结论；未取消评估。',
        'task_changed':'等待期间任务记录已变化或不可用，停止等待；不能认定检查通过。',
        'records_unavailable':'等待的评估明细已不可用，停止等待；不能认定检查通过。'}[wait['status']]
    return ['- 收尾等待：'+str(wait['elapsed_seconds'])+' 秒（最多 '+str(wait['max_seconds'])+' 秒）；'+text]
...
def brief(report):
    """Fixed user-facing snapshot; never infer per-turn counts, completion or causal correction."""
    if report['status']=='task_unavailable':
        return '### Navi 监督报告\n\n- 状态：任务记录不可用，无法核验监督状态；不能认定检查通过。\n'
    reasons={
        'review_budget_exhausted':'累计评估预算耗尽', 'review_in_progress':'已有评估排队或运行',
        'recording_or_input_capture_disabled':'记录或输入采集未启用', 'capture_gap':'存在采集缺口',
        'action_capture_disabled':'动作采集未启用', 'supervisor_disabled':'自动监督未启用',
        'reminders_disabled':'提醒未启用', 'history_not_ready':'历史上下文不可用',
        'context_sync_unavailable':'增量同步不可用'}
    runtime=report.get('runtime',{});blocks=runtime.get('unavailable_reasons',[])
    if runtime.get('eligible'):
        status='可调度（有新证据且满足间隔时评估）'
    elif blocks==['review_in_progress']:
        status='评估进行中'
    elif blocks:
        status='暂停或不可用：'+'、'.join(reasons.get(r,r) for r in blocks)
    else:
        status='运行条件未核验'
    rows=report['reviews'];completed=[r for r in rows if r['status']=='completed']
    counts=Counter(r['status'] for r in rows);fresh=Counter(r['freshness'] for r in completed)
    if any(r['health']=='overdue_or_worker_lost' for r in rows):
        status+='；有未结束评估已超时或执行进程失联，需核查'
    config=report['supervisor_configuration'];b=report['budget'];u=report['usage'];c=report['collection']
    interval=budget.limits({'supervisor':config})[0]
    ceiling='未设累计上限' if b.get('review_limit') is None else str(b['review_limit'])+' 次'
    scope=sum(bool(r['scope']) and r['scope']['verdict']=='drift' for r in completed if r['freshness']=='current')
    risk=sum(bool(r['implementation_risk']) and r['implementation_risk']['verdict']=='concern' for r in completed if r['freshness']=='current')
    uncertain=sum(bool(r['scope']) and r['scope']['verdict']=='uncertain' for r in completed if r['freshness']=='current')
    omitted=report['retention']['omitted_reviews']
    files=report['files'];unreadable=Counter(f['current_status'] for f in files if f['current_status'] not in TEXT_STATES)
    missing=len(u['usage_missing_review_ids'])+len(u['usage_incomplete_review_ids'])+sum(u.get('omitted_review_usage',{}).get(k,0) for k in ('missing','incomplete'))
    lines=['### Navi 监督报告','',
        '快照：'+when(report['generated_at'])+'；口径：当前对话累计用量与保留评估，非本轮统计。','',
        '- 状态：'+safe(status)+'。',
        '- 配置：'+safe(config.get('model'))+' / '+safe(config.get('effort'))+'；自动评估最小间隔 '+str(interval)+' 秒。',
        '- 预算：'+ceiling+'；累计已预留 '+str(b['reviews_used'])+' 次（包含失败调用的预留）。',
        '- 评估：保留 '+str(len(rows))+' 条，完成 '+str(len(completed))+'、失败 '+str(counts['failed'])+'、未结束 '+str(counts['queued']+counts['running'])+'、调用前取消/失效 '+str(counts['cancelled_before_call']+counts['superseded_before_call'])+'；已移出 '+str(omitted)+' 条旧明细。',
        '  - 完成结果：当前有效 '+str(fresh['current'])+'、过期 '+str(fresh['stale'])+'、历史活动 '+str(fresh['historical_observations'])+'、未复核 '+str(fresh['not_revalidated'])+'。',
        '- 发现：当前有效结果中，范围偏移 '+str(scope)+'、实现风险 '+str(risk)+'、范围判断不确定 '+str(uncertain)+'；这些计数可重叠。']
    lines[5:5]=final_wait_lines(report)
    findings=[r for r in completed if (r['scope'] and (r['scope']['verdict']=='drift' or
              r['scope']['verdict']=='uncertain' and review.DELIVERY_LABELS.intersection(r['scope']['scope_labels'])))
              or r['implementation_risk'] and r['implementation_risk']['verdict']=='concern']
    rank={'current':0,'historical_observations':1,'not_revalidated':2,'stale':3}
    findings.sort(key=lambda r:(rank.get(r['freshness'],4),-(r.get('created_at') or 0)))
    for row in findings[:3]:
        axes=[('范围',row['scope']),('实现风险',row['implementation_risk'])]
        for name,axis in axes:
            if not axis or axis['verdict'] not in ('drift','concern','uncertain'):continue
            if axis['verdict']=='uncertain' and not review.DELIVERY_LABELS.intersection(axis.get('scope_labels',[])):continue
            reason=axis['reason'];excerpt=reason[:160]+('…（详见完整报告）' if len(reason)>160 else '')
            categories='、'.join(label(x) for x in axis.get('scope_labels',axis.get('labels',[])))
            lines.append('  - '+safe(row['id'])+' / '+label(row['freshness'])+' / '+name+' '+label(axis['verdict'])+
                         ' '+categories+'：'+safe(excerpt))
    if len(findings)>3:lines.append('  - 另有 '+str(len(findings)-3)+' 条有发现的评估，详见完整报告。')
    paths=c['watched_files'];names='、'.join(safe(p) for p in paths[:3])
    if len(paths)>3:names+=' 等'
    gaps=['未观察文件和完整依赖未核验']
    if not c['current_files_revalidated']:gaps.append('当前文件未重新读取')
    if c['capture_gap']:gaps.append('采集存在缺口')
    snapshot_reasons={'oversized':'超过快照大小上限','binary':'二进制内容','unsafe_path':'路径不安全',
                      'not_regular':'非常规文件','changed_during_read':'读取期间发生变化',
                      'unreadable_or_symlink':'无法读取或为符号链接'}
    if unreadable:gaps.append('文件快照不可用 '+str(sum(unreadable.values()))+' 个（'+
                             '、'.join(snapshot_reasons.get(k,k)+' '+str(v)+' 个' for k,v in sorted(unreadable.items()))+'）')
    if c['dropped_tool_events']:gaps.append('累计已移出 '+str(c['dropped_tool_events'])+' 条工具事件')
    if c['truncated_tool_events']:gaps.append('保留工具事件中 '+str(c['truncated_tool_events'])+' 条内容被截断')
    activity='开启有界工具活动观察' if config.get('include_activity') else '未开启工具活动观察'
    lines+=['- 覆盖：指定观察文件 '+str(len(paths))+' 个'+('（'+names+'）' if names else '')+'；'+activity+'。缺口：'+'；'.join(gaps)+'。',
        '- 提醒：累计尝试 '+str(b['reminders_used'])+' 次；保留记录中确认接收 '+str(sum(n['delivery']=='confirmed_by_external_test_observer' for n in report['reminders']))+' 次；不代表已纠正。',
        '- 用量：累计已取得的独立评估输入＋输出 '+str(u['reported_input_plus_output'])+' tokens；用量缺失或不完整 '+str(missing)+' 条。历史整理与主 Agent 用量不计入；生成本报告不调用模型。',
        '', '当前有效结果为零或未发现问题，都不代表整轮任务通过；过期也不代表问题已修复。','']
    return '\n'.join(lines)
...
def markdown(report):
    ...
    lines=['# Navi 任务报告','',
        ...
        '## 任务依据','']
    lines[-2:-2]=final_wait_lines(report)
    gate=report.get('review_gate')
    if gate:
        if gate['reason']=='review_in_progress':
            lines[-2:-2]=['- **独立评估进行中**：同一对话不并发启动另一次评估；执行 Agent 继续工作。']
        else:
            lines[-2:-2]=['- **监督评估暂停**：达到用户设置的累计评估预算；调整预算后才可继续评估。执行 Agent 继续工作。']
...
def main():
    ...
    parser.add_argument('--format',choices=('json','markdown','brief'),default='markdown')
    parser.add_argument('--recorded-only',action='store_true')
    parser.add_argument('--final',action='store_true',help='Wait up to 15 seconds for existing reviews at task completion; never starts a review')
    parser.add_argument('--output',type=Path)
    args=parser.parse_args()
    try:
        report=generate(args.data_directory,args.session,args.recorded_only,final=args.final)
        rendered=(json.dumps(report,ensure_ascii=False,indent=2)+'\n' if args.format=='json'
                  else report['user_summary'] if args.format=='brief' else markdown(report))
        if args.output:
