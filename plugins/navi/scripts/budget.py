...
def limits(state):
    return (300, 4, 300) if state.get('supervisor', {}).get('profile') == 'long-task' else (30, 6, 60)
...
def review_gate(state, now=None):
    now = time.time() if now is None else now
    if remaining(state) == 0:
        return {'reason': 'review_budget_exhausted', 'resumes_at': None}
    times = sorted(t for t in state['budget'].get('review_times', []) if now-t < 3600)
    hourly = limits(state)[1]
    if len(times) >= hourly:
        return {'reason': 'hourly_review_limit', 'resumes_at': times[-hourly]+3600}
    return None
