...
def ready(state):
    ...
    if (state or {}).get('onboarding') and not active(state) and not state['sources']:
        return False
...
def assemble(state):
    ...
    anchor=history.get('bootstrap_anchor')
    anchored=bool(state.get('onboarding') and anchor and any(
        m['role']=='user' and all(m.get(k)==anchor.get(k) for k in ('id','turn_id','text_digest'))
        for m in history['messages']))
    if (not matches and not anchored) or matches != [s['id'] for s in live[:len(matches)]]:
