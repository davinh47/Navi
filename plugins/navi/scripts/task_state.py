...
def set_contract(state, value, source, proposal_id=None):
    previous = state["contract"] or None
...
def on_hook(data, payload):
    ...
    with locked(data, session) as (path, state):
        ...
        if state is None:
            ...
            if proposal is None or proposal["base_revision"] != state["contract"].get("revision", 0):
...
def agent_write(data, session, operation, value):
    with locked(data, session) as (path, state):
        ...
        if operation == "interpret":
            ...
            if value["base_revision"] != state["contract"].get("revision", 0):
            ...
            if not current:
                raise ValueError("no established contract for progress")
