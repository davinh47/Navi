...
class SupervisorTests(unittest.TestCase):
    ...
    def test_long_task_profile_preserves_spent_budget_and_throttles_hourly(self):
        ...
        self.assertIsNone(self.state()['budget']['reminder_limit'])
        self.cooldown();self.change('title=later\nbackend=wired\n')
        state=self.state();import time
        state['supervisor']['dispatch_times']=[time.time()]*4;self.save(state)
        before=len(self.launched);self.dispatch()
        self.assertEqual(len(self.launched),before)
        self.assertIn('hourly_limit',self.state()['supervisor']['last_status'])
        state=self.state();state['supervisor']['dispatch_times']=[time.time()-3601]*4;self.save(state)
        self.dispatch();self.assertEqual(len(self.launched),before+1)
    ...
    def test_standard_hourly_limit_recovers_and_profile_switch_does_not_reset(self):
        import time
        now=time.time()
        for i in range(6):
            with patch('time.time',return_value=now+i*61):
                self.change('title='+str(i)+'\nbackend=wired\n')
                self.dispatch();self.run_worker()
        with patch('time.time',return_value=now+400):
            self.change();self.dispatch()
            self.assertEqual(len(self.launched),6)
            self.assertEqual(supervisor.status(self.data,self.session)['review_gate']['reason'],'hourly_review_limit')
            supervisor.configure(self.data,self.session,False)
            supervisor.configure(self.data,self.session,True,profile='long-task')
            self.dispatch();self.assertEqual(len(self.launched),6)
        with patch('time.time',return_value=now+3601):
            supervisor.configure(self.data,self.session,True,profile='standard')
            self.dispatch();self.run_worker()
        self.assertEqual(self.state()['budget']['reviews_used'],7)
    ...
    def test_dispatch_failures_are_hourly_bounded_not_lifetime_bounded(self):
        import time
        now=time.time()
        def fail(*args):raise OSError('fixture')
        for i in range(6):
            with patch('time.time',return_value=now+i*31):
                self.change('title='+str(i)+'\nbackend=wired\n')
        ...
        with patch('time.time',return_value=now+3601):
            self.dispatch();self.run_worker()
        self.assertEqual(self.state()['supervisor']['dispatches'],7)
        self.assertEqual(self.state()['budget']['reviews_used'],1)

    def test_manual_reservations_between_dispatch_and_worker_defer_without_losing_candidate(self):
        import time
        now=time.time()
        with patch('time.time',return_value=now):
            self.change();self.dispatch()
            for i in range(6):
                evidence={'coverage':'partial','actions':[{'id':'manual','kind':'code','path':'x','evidence':str(i),'origin':'test'}]}
                review.reserve(self.data,self.session,evidence)
            self.run_worker(lambda *a,**kw:self.fail('hourly gate must precede model call'))
            self.assertEqual(self.state()['supervisor']['last_status'],'hourly_review_limit')
            self.assertIsNone(self.state()['supervisor']['last_attempt'])
        with patch('time.time',return_value=now+3601):
            self.dispatch();self.run_worker()
