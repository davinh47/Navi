...
For a longer task, say “Use Navi's long-task mode to supervise this task.” Navi uses the current task's
requirements from this conversation, including earlier context, unless you restrict that access.
You do not need a separate history instruction or repeated task description.

Codex imports validated conversation records, sets up supervision and confirms its status. If the host
cannot provide supported records, it reports the specific gap rather than inventing your requirements. Scope reminders go to Codex; reports go to you as a fixed, program-generated summary.
At task completion, the final report gives an existing review up to 15 seconds to return; progress reports
do not wait. This does not launch an extra review or guarantee that the latest edits were covered.
See [usage and troubleshooting](docs/USAGE.md) and [report fields](docs/REPORTING.md).

## Budgets and cost

| Profile | Minimum automatic dispatch interval | Concurrent reviews per conversation | Reminder cooldown |
| --- | ---: | ---: | ---: |
| Standard | 30 seconds | 1 | 1 minute |
| Long-task | 3 minutes | 1 | 5 minutes |

**There are no hourly review/dispatch caps or default cumulative quotas.** Automatic reviews require
new eligible evidence and minimum spacing; this is not a periodic schedule or a guarantee of complete
coverage. One review at a time per conversation includes manual reviews and automatic followups;
other conversations have independent slots. The executing agent continues while a review runs.
After spacing and concurrency gates clear, automatic work resumes at the next eligible native event.
With continuous new evidence, long-task spacing permits about 20 automatic reviews per hour; actual
token usage depends on the evidence and model. Reminders have separate deduplication and cooldown.
...
- **338 offline tests** cover state, scheduling, evidence, reminders, reports, rollback and package relocation.
