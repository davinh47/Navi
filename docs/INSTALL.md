...
4. Actual review model, profile, remaining budget, and `runtime.eligible`.
5. History integration and `context_sync` freshness if earlier conversation context was requested.
...
Reload the host/session after updating. Disable the plugin in the host to stop its hooks. The local
