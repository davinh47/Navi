...
There are no required semantic keywords for normal requirements. Asking Navi to supervise this task
includes bounded reading of its requirements in the current conversation, unless you restrict history
access. The skill configures workspace input capture, task action capture and automatic supervision through
their separate runtime controls. You do not need to repeat the task or add a history-reading instruction.

The enabling message can precede live capture. `history.py bootstrap` creates a pending task shell and
uses the next native hook to obtain the real conversation/turn boundary. A detached reader validates and
imports original messages, then enables raw-source context without an extra history-model call. It does
not synthesize a user submission. Unsupported or unavailable evidence remains a visible gap.

For multi-hour work, explicitly select `long-task`. An optional separately requested history
interpretation has a one-call budget; a failed interpretation can use validated originals while preserving
that failure in reports. Incremental local synchronization refreshes supporting dialogue locally.
...
Continuous supervision uses new evidence, minimum automatic dispatch spacing (30 seconds in standard,
180 seconds in long-task) and one active review per conversation. There are no hourly review/dispatch
caps or default cumulative quotas. Manual reviews and automatic followups share the same concurrency
slot and any explicit total budget; different conversations remain independent. Deferred manual requests
return without reserving usage and are not automatically queued. Automatic work can resume at the next
eligible event, without a background polling loop. Reminders have separate cooldown, deduplication and
expiry checks (60-second reminder cooldown in standard, 300 seconds in long-task). A review already in
progress is shown separately from exhausted user-set budget.
...
To remove that ceiling while keeping automatic spacing, concurrency control and all spent counters:
...
An explicitly saved ceiling persists until changed; updating the plugin or asking the agent to continue
is not a request to remove it. `limit_origin=explicit` records a command-set value, not independent proof
of user consent. The executing agent must follow the latest user decision and must not restore an older
ceiling from a summary. Configuration changes made in another conversation are not automatically imported
as user instructions here.
...
The agent should paste the fixed `--format brief` summary verbatim into its response. It shows current
readiness and budget, retained review counts and freshness, findings, concrete capture gaps, reminder
attempts and known review usage. Counts explicitly refer to cumulative conversation usage or retained
records, not necessarily the current turn. A zero count is not a whole-task clearance. `--format json`
contains the same text in `user_summary`; `--format markdown` keeps the full evidence report.
The current fixed summary is Chinese and has no locale selector. The skill requires verbatim inclusion;
the host does not enforce the agent's final wording. See [report fields and troubleshooting](REPORTING.md)
for count definitions, freshness, concrete coverage gaps and unfinished-review handling.
For a final task-completion report, append `--final` once. It waits up to 15 seconds only for existing
reviews, then reports available results and any timeout. Progress reports and user-continuation handoffs
do not wait. No new review or retry is launched, and normal stale-result checks still apply.
For direct CLI use with actual paths/session values:

```sh
python3 "$PLUGIN_ROOT/scripts/report.py" \
  --data-directory "$PLUGIN_DATA" --session "$NAVI_SESSION_ID" --format brief
python3 "$PLUGIN_ROOT/scripts/report.py" \
