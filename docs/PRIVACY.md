...
Task input capture requires workspace opt-in. Asking Navi to supervise the current task authorizes its
normal bounded observation and same-conversation requirement import, subject to explicit user restrictions.
The skill enables the separate capture/history controls for that task; installation alone does not. Opted-in records can contain user messages, source code, absolute
...
Both the fixed brief and the full report can disclose private information. The brief includes watched
paths and excerpts of model reasons; full Markdown/JSON can include user sources and observed code.
JSON also includes the same brief in `user_summary`. Rendering uses local state and, when allowed,
refreshes explicit file snapshots in memory; it does not send a new model request. Including the report
in the main agent's conversation still makes that text part of the host's conversation context.
